...
namespace Engine::Math::Simd_SSE
{
	...
	__forceinline float HorizontalAdd3(NuVec4 v)
	{
		NuVec4 shuf = _mm_movehdup_ps(v);      
		NuVec4 sums = _mm_add_ps(v, shuf);      
	...
	{
		a = _mm_and_ps(a, _mm_castsi128_ps(_mm_set_epi32(0, -1, -1, -1))); // w = 0
		b = _mm_and_ps(b, _mm_castsi128_ps(_mm_set_epi32(0, -1, -1, -1))); // w = 0
		return HorizontalAdd3(_mm_mul_ps(a, b));
	}
	...
	inline float Determinant(const NuMat4& m)
	{
	...
	inline NuMat4 Inverse(const NuMat4& m)
	{
