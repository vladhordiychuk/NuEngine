...
namespace NuEngine::Math
{
    ...
    public:
        ...
        {
            NuEngine::Core::Types::NuAssert(list.size() == 3, "Vector3 initializer_list must contain exactly 3 elements!");
            auto it = list.begin();
            NuFloat x = *it++;
            NuFloat y = *it++;
            NuFloat z = *it++;
         ...
         * @return A new Vector3 that is the result of the conversion.
         */
        explicit NU_FORCEINLINE Vector3(const Vector2& vec, NuFloat z = 0.0f) noexcept;
        ...
        [[nodiscard]] NU_FORCEINLINE Vector3 operator/(NuFloat scalar) const noexcept
        {
            NuEngine::Core::Types::NuAssert(std::fabs(scalar) > std::numeric_limits<NuFloat>::epsilon(), "Vector3 division by zero or near zero!");
        ...
        {
            NuEngine::Core::Types::NuAssert(index >= 0 && index < 3, "Vector3 index out of bounds! Valid range: 0..2");
        ...
        */
        ...
        /**
         * @brief Performs linear interpolation towards another 3D vector.
         *
         ...
         */
        [[nodiscard]] NU_FORCEINLINE Vector3 Lerp(Vector3 other, NuFloat t) const noexcept
        {
...
#pragma warning(disable: 4201)
        ...
        union
        {
            ...
            NuVecStorage4 m_storage;         // Storage for SIMD operations
        };
#if defined(_MSC_VER)
#pragma warning(pop)
#endif
...
namespace NuEngine::Math
{
    explicit NU_FORCEINLINE Vector3::Vector3(const Vector4& vec) noexcept
        : m_data(VectorAPI::Set(vec.X(), vec.Y(), vec.Z(), 0.0f))
    {
    }

    explicit NU_FORCEINLINE Vector3::Vector3(const Vector2& vec, NuFloat z) noexcept
        : m_data(VectorAPI::Set(vec.X(), vec.Y(), z, 0.0f))
    {
    }
}