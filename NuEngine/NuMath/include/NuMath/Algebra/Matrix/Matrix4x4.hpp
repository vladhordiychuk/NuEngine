...
namespace NuMath
{
	...
	public:
		 ...
		 */
		NU_FORCEINLINE Matrix4x4(float m00, float m01, float m02, float m03,
		 ...
		 */
		NU_FORCEINLINE Matrix4x4(std::initializer_list<float> list) noexcept
		{
			NU_MATH_ASSERT(list.size() == 16, "Matrix4x4 initializer_list must contain exactly 16 elements!");

			auto it = list.begin();

			Vector4 r0(it[0], it[1], it[2], it[3]);
			Vector4 r1(it[4], it[5], it[6], it[7]);
			Vector4 r2(it[8], it[9], it[10], it[11]);
			Vector4 r3(it[12], it[13], it[14], it[15]);
...
         */
		 ...
		 */
		[[nodiscard]] NU_FORCEINLINE  static Matrix4x4 Identity() noexcept
		{
			return Matrix4x4(MatrixAPI::SetIdentityMatrix());
		}
		 ...
		 * @return A new matrix.
		 */
		[[nodiscard]] NU_FORCEINLINE static Matrix4x4 FromRows(const Vector4& row0, const Vector4& row1, const Vector4& row2, const Vector4& row3) noexcept
		{
			return Matrix4x4(MatrixAPI::FromRows(row0.SimdData(), row1.SimdData(), row2.SimdData(), row3.SimdData()));
		}
		 ...
		 * @return A new matrix.
		 */
		[[nodiscard]] NU_FORCEINLINE static Matrix4x4 FromColumns(const Vector4& col0, const Vector4& col1, const Vector4& col2, const Vector4& col3) noexcept
		{
			return Matrix4x4(MatrixAPI::FromColumns(col0.SimdData(), col1.SimdData(), col2.SimdData(), col3.SimdData()));
		}
		...
		{
			return Vector4(MatrixAPI::Mul(m_data, other.SimdData()));
		}
...
         */
		[[nodiscard]] NU_FORCEINLINE bool operator==(const Matrix4x4& other) const noexcept
		{
...
         */
		 ...
		 */
		[[nodiscard]] NU_FORCEINLINE static Matrix4x4 CreateTranslation(const Vector3& translation) noexcept
		{
			return Matrix4x4(MatrixAPI::CreateTranslation(translation.SimdData()));
		}
		 ...
		 */
		[[nodiscard]] NU_FORCEINLINE static Matrix4x4 CreateScale(const Vector3& scale) noexcept
		{
			return Matrix4x4(MatrixAPI::CreateScale(scale.SimdData()));
		}
		 ...
		 */
		[[nodiscard]] NU_FORCEINLINE static Matrix4x4 CreateLookAt(const Vector3& eye, const Vector3& target, const Vector3& up) noexcept
		{
			return Matrix4x4(MatrixAPI::CreateLookAt(eye.SimdData(), target.SimdData(), up.SimdData()));
		}
		 ...
		 */
		[[nodiscard]] NU_FORCEINLINE static Matrix4x4 CreatePerspective(float fovY, float aspect, float nearZ, float farZ) noexcept
		{
		 ...
		 */
		[[nodiscard]] NU_FORCEINLINE static Matrix4x4 CreateOrthographic(float left, float right, float bottom, float top, float nearZ, float farZ) noexcept
		{
		 ...
		 */
		[[nodiscard]] NU_FORCEINLINE Vector4 GetColumn(int index) const noexcept
		{
		 ...
		 */
		[[nodiscard]] NU_FORCEINLINE Vector4 GetRow(int index) const noexcept
		{
		 ...
		 *
		 * @param index The index of the column to set (0-3).
		 * @param column The vector containing the new values for the column.
		 */
		NU_FORCEINLINE void SetColumn(int index, const Vector4& column) noexcept
		{
		 ...
		 *
		 * @param index The index of the row to set (0-3).
		 * @param row The vector containing the new values for the row.
		 */
		NU_FORCEINLINE void SetRow(int index, const Vector4& row) noexcept
		{
